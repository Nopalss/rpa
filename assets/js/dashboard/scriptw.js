$(document).ready(function () {
    ...
    function enqueueChartRequest(site, forceRefresh = false) {
        ...
        // 🔥 FIX: forceRefresh HARUS TEMBUS SEMUA BLOCK
        if (!forceRefresh && window.cachedChartData[site]) {
            return;
        }

        if (!forceRefresh && (window.loadingCharts[site] || window.inFlightRequests[site])) {
            return;
        }

        // 🔥 kalau forceRefresh → reset paksa
        if (forceRefresh) {
            window.loadingCharts[site] = false;
            window.inFlightRequests[site] = false;
        }

        window.loadingCharts[site] = true;
        window.inFlightRequests[site] = true;
    ...
    //     // 🔥 FIX UTAMA
    //     if (window.loadingCharts[site] || window.inFlightRequests[site]) return;
...
`);
    ...
    function staggerRefreshAllSites(intervalMs = 30000) {
