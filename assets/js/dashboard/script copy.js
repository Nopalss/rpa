$(document).ready(function () {
    ...
    const MAIN_SLOTS = 4;
    const CACHE_TTL = 5 * 60 * 1000; // 3 menit
    const DASHBOARD_NAMESPACE = 'normal';
    ...
    function getCachedChart(site) {
        ...
        if (mem) {
            ...
            return null; // ⛔ jangan pakai cache expired
        }
    ...
    function isCacheExpired(site) {
        ...
        const ttlExpired =
            (Date.now() - mem.fetchedAt) >= CACHE_TTL;

        const prodDateChanged =
            mem.productionDate !== getCurrentProductionDate();

        return ttlExpired || prodDateChanged;
    }
    ...
    function saveChartCache(site, data) {
        const payload = {
            data,
            fetchedAt: Date.now(),
            productionDate: getCurrentProductionDate() // ✅ PAKAI CLIENT
        };
    ...
    async function runMainCarousel() {
        ...
        for (let mainIdx = 0; mainIdx < MAIN_SLOTS; mainIdx++) {
            ...
            if (cached) {
                ...
                // 🔥 JANGAN override chart yang sudah ada
                const hasChart =
    ...
    async function processChartQueue() {
        if (window.chartApiBusy) return;
        if (window.chartApiQueue.length === 0) return;

        window.chartApiBusy = true;
        ...
        try {
            ...
            console.error('[QUEUE] Error loading', site, e);

        } finally {

            const failCount = window.apiFailCount[site] || 0;
            const delay = Math.min(1200 * (failCount + 1), 15000);
    ...
    function loadHistogramChartInternal(site, forceRefresh = false) {
        return new Promise((resolve, reject) => {
            ...
            // Pakai cache hanya jika BELUM expired
            if (!forceRefresh && !isCacheExpired(actual)) {
                const cached = getCachedChart(actual);
                if (cached) {
                    window.cachedChartData[actual] = cached;
                    resolve();
                    return;
                }
            }
