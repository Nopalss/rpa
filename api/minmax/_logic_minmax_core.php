...
// Production Date
// ======================================================
if (!function_exists('get_production_date')) {
...
if (!isset($pdo) || !($pdo instanceof PDO)) {
    throw new RuntimeException('PDO not available in minmax core logic');
}
// ======================================================
// Input
// ======================================================
...
$user_id = $GLOBALS['__engine_user_id__'] ?? null;
...
if ($user_interval_width <= 0) {
    return ['success' => false, 'message' => 'interval_width invalid'];
}
...
if ($start_col !== null && $end_col !== null) {
    ...
    if ($start_col < 1 || $end_col > $TABLE2_MAX || $start_col > $end_col) {
        return ['success' => false, 'message' => 'Range column invalid'];
    }
    ...
    // ---------- tbl_data2 ----------
    if ($end_col > $TABLE1_MAX && $line_id > 0 && $application_id > 0) {
    ...
    return ['success' => false, 'message' => 'Mode lama tidak aktif'];
    // exit;
}
...
// exit;
// SIMPAN KE GLOBAL AGAR TIDAK MEMATIKAN WORKER
$GLOBALS['__minmax_result__'] = $output;
// ------------------ Helper functions (Excel-accurate Normal CDF) ------------------


if (!function_exists('normal_cdf')) {
...
if (!function_exists('std_normal_cdf')) {
