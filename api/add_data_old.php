...
try {
    ...
    try {
        ...
        // -------------------------
        // Fallback: hapus data pertama jika insert ke-2 gagal
        // -------------------------
        $deleteStmt = $pdo->prepare("DELETE FROM tbl_data WHERE record_no = :rec");
        $deleteStmt->execute([':rec' => $record_no]);

        throw new Exception("Insert tbl_data2 gagal, rollback manual dilakukan: " . $e2->getMessage());
    }
